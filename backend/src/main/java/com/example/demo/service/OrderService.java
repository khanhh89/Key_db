...
public class OrderService {
    ...
    public List<OrderEntity> getAllOrders(String adminAuth) {
    ...
    public OrderEntity maskOrder(OrderEntity original) {
    ...
    public OrderEntity createOrder(HttpServletRequest request, OrderEntity orderReq) {
    ...
    public Optional<OrderEntity> findOrder(String key) {
    ...
    public ResponseEntity<OrderEntity> getOrderStatus(String id) {
        ...
        if ("PENDING".equalsIgnoreCase(order.getStatus())
            ...
            System.out.println(">>> [OrderService] Instantly deleted expired order (>15m) from DB: " + order.getId());
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<?> verifyCustomerPayment(String id) {
        Optional<OrderEntity> orderOpt = findOrder(id);
        if (orderOpt.isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        ...
        if ("PENDING".equalsIgnoreCase(order.getStatus())
                ...
                && order.getCreatedAt().isBefore(LocalDateTime.now(java.time.ZoneId.of("Asia/Ho_Chi_Minh")).minusMinutes(15))) {
            orderRepository.delete(order);
            System.out.println(">>> [OrderService] Instantly deleted expired order (>15m) on verify: " + order.getId());
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<?> confirmOrderPayment(String adminAuth, String id) {
        if (!AdminSecurityUtil.isValidAdmin(adminAuth)) {
    ...
    public boolean checkAndUpdatePayosStatus(OrderEntity order) {
        if ("PAID".equals(order.getStatus())) {
            return true;
        }
    ...
    public void fulfillOrderKey(OrderEntity order) {
        fulfillOrderKeyStatic(order, licenseKeyRepository, orderRepository);
        try {
            emailService.sendKeyEmail(order);
        } catch (Exception e) {
            System.err.println(">>> Failed to send email in OrderService: " + e.getMessage());
        }
    }

    public ResponseEntity<?> deleteOrder(String adminAuth, String id) {
        if (!AdminSecurityUtil.isValidAdmin(adminAuth)) {
    ...
    public ResponseEntity<?> clearAllOrders(String adminAuth) {
        if (!AdminSecurityUtil.isValidAdmin(adminAuth)) {
